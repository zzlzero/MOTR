...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        self.vid_root = os.path.join(self.save_path, "video.mp4")
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100, vis=False):
        ...
        # videowriter = cv2.VideoWriter(self.vid_root, cv2.VideoWriter_fourcc('M','J','P','G'), 30, tqdm(loader)[1][1], tqdm(loader)[1][2])
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            if vis:
                ...
                gt_boxes = None
                vis_img = self.visualize_img_with_bbox(cur_vis_img_path, ori_img, dt_instances, ref_pts=all_ref_pts, gt_boxes=gt_boxes)
